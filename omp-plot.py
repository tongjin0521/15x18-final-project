...
for source in csvs:
    ...
    data = pd.read_csv(source + "-5000-scheduling.csv")
    source = source + "-5000-scheduling-"
    ...
    for method in ["auto", "static", "dynamic", "guided"]:
        data = get_data_a(method, 10, "cleavland.csv")
        x = data["AGENTNUM"]
        y = data["TIME"]["mean"]
        yerr = data["TIME"]["std"]
        plt.errorbar(x, y, label=method, marker="o", capsize=3)
    plt.xlabel("AGENTNUM")
    plt.ylabel("TIME")
