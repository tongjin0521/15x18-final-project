...
void standardize(vector<vector<double>>& input) {
    ...
    for (size_t i = 0; i < input[0].size(); i++) {
        double sum = accumulate(input.begin(), input.end(), 0.0f, [i](double a, const vector<double>& b) { return a + b[i]; });
        double mean = sum / input.size();

        // Calculate the standard deviation of the column
        double sq_sum = inner_product(input.begin(), input.end(), input.begin(), 0.0f, plus<>(), [i, mean](const vector<double>& a, const vector<double>& b) { return (b[i] - mean) * (b[i] - mean); });
